package com.kyonggi.diet.review.favoriteReview.service;

import com.kyonggi.diet.review.favoriteReview.DTO.FavoriteKyongsulFoodReviewDTO;

import java.util.List;

public interface FavoriteKyongsulFoodReviewService {
    /**
     * 관심 음식 리뷰 생성 메서드
     * @param reviewId (Long)
     * @param email    (String)
     */
    void createFavoriteKyongsulReview(Long reviewId, String email);

    /**
     * 관심 음식 리뷰 DTO 조회 메서드
     *
     * @param id (Long)
     * @return FavoriteKyongsulFoodReviewDTO
     */
    FavoriteKyongsulFoodReviewDTO findById(Long id);

    /**
     * 관심 경슐랭 음식 리뷰 DTO 전체 조회 메서드
     * @return List<FavoriteKyongsulFoodReviewDTO>
     */
    List<FavoriteKyongsulFoodReviewDTO> findAll();

    /**
     * 몀버별 관심 음식 리뷰 DTO 조회 메서드
     * @param email (String)
     * @return List<FavoriteKyongsulFoodReviewDTO>
     */
    List<FavoriteKyongsulFoodReviewDTO> findFavoriteKyongsulFoodReviewListByMember(String email);

    /**
     * 멤버별 관심 음식 리뷰 삭제
     * @param email    (String)
     * @param reviewId (Long)
     */
    void deleteFavoriteReview(String email, Long reviewId);

    /**
     * 해당 리뷰에 대한 좋아요 수 카운트 추출
     * @param reviewId (Long)
     * @return Long
     */
    Long getFavoriteReviewCountByReviewId(Long reviewId);
}
